(function (window, $) {
    ...
    function switchPanel(menu) {
        ...
        if (currentMenu.path === '/scheduled-tasks') {
            renderScheduledTasksScene();
            return;
        }
    ...
    function renderScheduledTasksScene() {
        destroyAllDashboardCharts();
        setHero(
            '定时任务',
        ...
        renderOverviewCards([
            { label: '任务总数', value: scheduledTaskState.total || '-', icon: 'fas fa-clock', tone: 'tone-info', note: '所有已配置任务' },
            { label: '当前页码', value: scheduledTaskState.page, icon: 'fas fa-file-alt', tone: 'tone-success', note: '分页查询' },
            { label: '每页条数', value: scheduledTaskState.size, icon: 'fas fa-list-ol', tone: 'tone-warning', note: '默认每页10条' },
            { label: '权限标识', value: 'scheduledTask:manage', icon: 'fas fa-key', tone: 'tone-danger', note: '接口鉴权码' }
        ]);
        setPrimaryPanelTitle('定时任务列表');
        var searchHtml = '' +
            '<div class="user-search-bar">' +
            '<form id="task-search-form" class="form-inline" novalidate>' +
            '<div class="form-group mr-2 mb-2">' +
            '<input type="text" id="ts-task-name" class="form-control form-control-sm" placeholder="任务名称" maxlength="64" value="' + escapeHtml(scheduledTaskState.taskName) + '">' +
            '</div>' +
            '<div class="form-group mr-2 mb-2">' +
            '<input type="text" id="ts-task-group" class="form-control form-control-sm" placeholder="任务分组" maxlength="64" value="' + escapeHtml(scheduledTaskState.taskGroup) + '">' +
            '</div>' +
            '<div class="form-group mr-2 mb-2">' +
            '<select id="ts-status" class="form-control form-control-sm">' +
            '<option value="">全部状态</option>' +
            '<option value="1"' + (scheduledTaskState.taskStatus === 1 ? ' selected' : '') + '>运行中</option>' +
            '<option value="0"' + (scheduledTaskState.taskStatus === 0 ? ' selected' : '') + '>已暂停</option>' +
            '</select>' +
            '</div>' +
            '<button type="submit" class="btn btn-primary btn-sm mr-2 mb-2"><i class="fas fa-search mr-1"></i>查询</button>' +
        ...
        bindScheduledTaskEvents();
        fetchScheduledTaskPage();
    }

    function bindScheduledTaskEvents() {
        $(document).off('submit.taskSearch').on('submit.taskSearch', '#task-search-form', function (e) {
            e.preventDefault();
            scheduledTaskState.taskName = $.trim($('#ts-task-name').val());
            scheduledTaskState.taskGroup = $.trim($('#ts-task-group').val());
            var statusVal = $('#ts-status').val();
            scheduledTaskState.taskStatus = statusVal !== '' ? Number(statusVal) : null;
            scheduledTaskState.page = 1;
            fetchScheduledTaskPage();
        });
        $(document).off('click.taskReset').on('click.taskReset', '#ts-reset-btn', function () {
            scheduledTaskState.taskName = '';
            scheduledTaskState.taskGroup = '';
            scheduledTaskState.taskStatus = null;
            scheduledTaskState.page = 1;
            $('#ts-task-name').val('');
            $('#ts-task-group').val('');
            $('#ts-status').val('');
        ...
        $(document).off('click.taskDelete').on('click.taskDelete', '.btn-task-delete', function () {
            scheduledTaskState.pendingDeleteTaskId = $(this).closest('tr').data('id');
        ...
        $(document).off('click.taskDeleteConfirm').on('click.taskDeleteConfirm', '#task-delete-confirm-btn', function () {
            if (scheduledTaskState.pendingDeleteTaskId) {
                deleteScheduledTask(scheduledTaskState.pendingDeleteTaskId);
            }
            $('#task-delete-modal').modal('hide');
            scheduledTaskState.pendingDeleteTaskId = null;
        });
        $(document).off('click.taskPage').on('click.taskPage', '.task-page-btn', function () {
            var p = Number($(this).data('page'));
            if (p >= 1 && p <= scheduledTaskState.pages) {
                scheduledTaskState.page = p;
                fetchScheduledTaskPage();
            }
        });
        ...
        $(document).off('change.execLogStatus').on('change.execLogStatus', '#exec-log-status-filter', function () {
            var val = $(this).val();
            execLogState.executionStatus = val !== '' ? Number(val) : null;
            execLogState.page = 1;
            fetchExecLogPage();
        });
    }

    function fetchScheduledTaskPage() {
        var params = { page: scheduledTaskState.page, size: scheduledTaskState.size };
        if (scheduledTaskState.taskName) params.taskName = scheduledTaskState.taskName;
        if (scheduledTaskState.taskGroup) params.taskGroup = scheduledTaskState.taskGroup;
        if (scheduledTaskState.taskStatus !== null) params.taskStatus = scheduledTaskState.taskStatus;
        $.ajax({
            ...
            success: function (resp) {
                ...
                var page = resp.data;
                scheduledTaskState.total = page.total || 0;
                scheduledTaskState.pages = page.pages || 0;
                renderScheduledTaskTable(page.records || []);
                renderScheduledTaskPagination();
                updateScheduledTaskOverviewCards();
            }
        });
    }

    function renderScheduledTaskTable(records) {
        if (!records.length) {
    ...
    function renderScheduledTaskPagination() {
        var total = scheduledTaskState.total;
        var pages = scheduledTaskState.pages;
        var current = scheduledTaskState.page;
        if (pages <= 1) {
    ...
    function updateScheduledTaskOverviewCards() {
        renderOverviewCards([
            { label: '任务总数', value: scheduledTaskState.total || '-', icon: 'fas fa-clock', tone: 'tone-info', note: '所有已配置任务' },
            { label: '当前页码', value: scheduledTaskState.page, icon: 'fas fa-file-alt', tone: 'tone-success', note: '分页查询' },
            { label: '每页条数', value: scheduledTaskState.size, icon: 'fas fa-list-ol', tone: 'tone-warning', note: '默认每页10条' },
    ...
    function openExecLogModal(taskId, taskName) {
        execLogState.taskId = taskId;
        execLogState.taskName = taskName || '';
        execLogState.page = 1;
        execLogState.executionStatus = null;
    ...
    function fetchExecLogPage() {
        var params = { page: execLogState.page, size: execLogState.size };
        if (execLogState.executionStatus !== null) params.executionStatus = execLogState.executionStatus;
        $.ajax({
            url: '/api/scheduled-tasks/' + execLogState.taskId + '/execution-logs',
            method: 'GET',
            data: params,
            success: function (resp) {
                if (!resp || Number(resp.code) !== 0 || !resp.data) {
                ...
                var page = resp.data;
                execLogState.total = page.total || 0;
                execLogState.pages = page.pages || 0;
    ...
    function renderExecLogPagination() {
        var total = execLogState.total;
        var pages = execLogState.pages;
        var current = execLogState.page;
        if (pages <= 1) {
